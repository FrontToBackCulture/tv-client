...
import { SOURCE_OPTIONS } from "@/hooks/chat/useTaskAdvisor";
import type { AutomationType, DataSourceConfig } from "../types";
...
export function DataSourceConfigPanel({ config, automationType, onChange }: Props) {
  if (automationType === "dio") {
    return <DioSources config={config} onChange={onChange} />;
  }
  return <SkillPicker config={config} onChange={onChange} />;
}

// ---------------------------------------------------------------------------
// DIO: checkbox grid of data sources
// ---------------------------------------------------------------------------

function DioSources({ config, onChange }: { config: DataSourceConfig; onChange: (c: DataSourceConfig) => void }) {
  const sources = config.sources ?? { tasks: false, deals: false, emails: false, projects: false, calendar: false };
      ...
      {SOURCE_OPTIONS.map(({ key, label, desc }) => (
        <label key={key} className="flex items-start gap-3 py-1 cursor-pointer">
          <input
            type="checkbox"
            checked={sources[key]}
            onChange={(e) => onChange({ ...config, sources: { ...sources, [key]: e.target.checked } })}
...
// ---------------------------------------------------------------------------
// Skill: search + select skills
// ---------------------------------------------------------------------------

function SkillPicker({ config, onChange }: { config: DataSourceConfig; onChange: (c: DataSourceConfig) => void }) {
