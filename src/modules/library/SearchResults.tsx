...
export function SearchResults({
  ...
  if (results.length === 0) {
    return (
      <div className="p-4 text-center">
        <Search size={20} className="mx-auto mb-2 text-zinc-500 dark:text-zinc-600" />
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className={cn("text-sm truncate", isSelected ? "text-teal-600 dark:text-teal-400" : "text-zinc-700 dark:text-zinc-300")}>
                {result.name}
              </div>
              {result.preview && (
                <div className="text-xs text-zinc-500 truncate mt-0.5">
                  {result.line_number && (
                    <span className="text-zinc-500 dark:text-zinc-600">L{result.line_number}: </span>
                  )}
                  {result.preview}
                </div>
              )}
              <div className="text-xs text-zinc-500 dark:text-zinc-600 truncate mt-0.5">
                {result.path}
              </div>
            </div>
