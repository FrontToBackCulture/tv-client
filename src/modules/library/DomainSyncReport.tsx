          ...
          <div key={env}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {domains.map((domain) => (
                <div
                  key={domain.name}
                  className="p-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl"
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <Database size={16} className="text-teal-600 dark:text-teal-400" />
                      <span className="font-medium text-zinc-800 dark:text-zinc-200">{domain.name}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-4 text-xs text-zinc-500">
                    <span>
                      <span className="text-teal-600 dark:text-teal-400">{domain.summary.totalExtractions}</span> extracted
                    </span>
                    <span>
                      <span className="text-green-600 dark:text-green-400">{domain.summary.totalGenerated}</span> generated
                    </span>
                    {domain.audit?.totalStale ? (
                      <span>
                        <span className="text-amber-600 dark:text-amber-400">{domain.audit.totalStale}</span> stale
                      </span>
                    ) : null}
                  </div>
                </div>
  ...
  return (
    <div className="space-y-8">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  {type === "queries" && <FileCode size={16} className="text-blue-600 dark:text-blue-400" />}
                  {type === "workflows" && <Workflow size={16} className="text-purple-600 dark:text-purple-400" />}
                  {type === "dashboards" && <LayoutDashboard size={16} className="text-amber-600 dark:text-amber-400" />}
                  {type === "tables" && <Database size={16} className="text-green-600 dark:text-green-400" />}
                  <span className="font-medium text-zinc-800 dark:text-zinc-200 capitalize">{type}</span>
                ...
                <div className="mb-2">
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-zinc-700 dark:text-zinc-300">{stats.detailsPct}%</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-zinc-500 dark:text-zinc-400">Explanations</span>
                    <span className="text-zinc-700 dark:text-zinc-300">{stats.explanationsPct}%</span>
                  </div>
                  <div className="h-2 bg-slate-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                    <div
      ...
      <div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 overflow-hidden overflow-x-auto">
          <table className="w-full min-w-[800px]">
              ...
              <tr className="border-b border-slate-200 dark:border-zinc-800 bg-slate-50 dark:bg-zinc-900/50">
                ...
                {artifactTypes.map((type) => (
                  <React.Fragment key={type}>
                    <th className="text-center text-xs text-zinc-500 dark:text-zinc-600 px-2 py-2">Details</th>
                    <th className="text-center text-xs text-zinc-500 dark:text-zinc-600 px-2 py-2">Expl.</th>
                  </React.Fragment>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.domains
                ...
                >
                  <td className="px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200">{domain.name}</td>
                  {artifactTypes.map((type) => {
                    ...
                    return (
                      <React.Fragment key={type}>
                        <td className="text-center px-2 py-3">
                          <span className={cn("text-sm", details?.count ? "text-zinc-700 dark:text-zinc-300" : "text-zinc-400 dark:text-zinc-600")}>
                            {details?.count || 0}
                          </span>
                        </td>
                        <td className="text-center px-2 py-3">
                          <span className={cn("text-sm", expl?.count ? "text-zinc-700 dark:text-zinc-300" : "text-zinc-400 dark:text-zinc-600")}>
                            {expl?.count || 0}
                          </span>
                        </td>
                      </React.Fragment>
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-4">
        <select
          ...
          className="px-3 py-1.5 bg-white dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 rounded text-sm text-zinc-700 dark:text-zinc-300"
        >
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 overflow-hidden overflow-x-auto">
        <table className="w-full min-w-[900px]">
          <thead>
            <tr className="border-b border-slate-200 dark:border-zinc-800">
              <th
                className="text-left text-xs font-medium text-zinc-500 uppercase tracking-wider px-4 py-3 cursor-pointer hover:text-zinc-700 dark:hover:text-zinc-300"
              ...
              <th
                className="text-right text-xs font-medium text-zinc-500 uppercase tracking-wider px-4 py-3 cursor-pointer hover:text-zinc-700 dark:hover:text-zinc-300"
              ...
              >
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-zinc-800 dark:text-zinc-200">{domain.name}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <span className={cn(
                    "text-xs px-2 py-0.5 rounded",
                    domain.environment === "production" ? "bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" :
                    domain.environment === "demo" ? "bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400" :
                    "bg-slate-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400"
                  )}>
                    {domain.environment}
                  </span>
                </td>
                ...
                <td className="px-4 py-3 text-right text-sm text-zinc-800 dark:text-zinc-200 font-medium">
          ...
          {sources.map((source) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <span className="font-medium text-zinc-800 dark:text-zinc-200">{source.tab}</span>
                <code className="text-xs px-2 py-1 bg-slate-100 dark:bg-zinc-800 rounded text-teal-600 dark:text-teal-400">
                  {source.file}
                </code>
              </div>
              <p className="text-sm text-zinc-500 mb-2">{source.description}</p>
              <code className="text-xs text-zinc-500 dark:text-zinc-600 break-all">{source.path}</code>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl p-4 space-y-3">
          <div>
            ...
            <code className="text-sm text-zinc-700 dark:text-zinc-300 break-all">{reportPath}</code>
          </div>
          <div>
            <div className="text-xs text-zinc-500 mb-1">all-audit-results.json</div>
            <code className="text-sm text-zinc-700 dark:text-zinc-300 break-all">{auditPath}</code>
          </div>
          <div>
            <div className="text-xs text-zinc-500 mb-1">Base Path</div>
            <code className="text-sm text-zinc-700 dark:text-zinc-300 break-all">{basePath}</code>
          </div>
        </div>
      </div>
    </div>
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-red-500 dark:text-red-400">Failed to load sync report</p>
          <p className="text-xs text-zinc-500 mt-1">
            Could not find sync-report.json
          </p>
          <code className="text-xs text-zinc-500 dark:text-zinc-600 mt-2 block">{reportPath}</code>
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={() => refetch()}
            className="flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-zinc-800 hover:bg-slate-200 dark:hover:bg-zinc-700 rounded-lg text-sm text-zinc-700 dark:text-zinc-300 transition-colors"
          >
            <RefreshCw size={14} />
            Refresh
          </button>
