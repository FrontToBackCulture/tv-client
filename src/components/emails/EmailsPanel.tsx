...
function EmailRow({ email, onUnlink }: { email: LinkedEmail; onUnlink: (id: string) => void }) {
  return (
      ...
      <div className="flex-1 overflow-auto">
        {isLoading ? (
          ...
          <>
            ...
            {linkedEmails.filter((e) => e.email_type === "correspondence").length > 0 && (
              <div>
                <div className="sticky top-0 z-10 flex items-center gap-1.5 px-4 py-1.5 bg-zinc-50/90 dark:bg-zinc-900/90 backdrop-blur-sm border-b border-zinc-100 dark:border-zinc-800/50">
                  ...
                  <span className="text-[10px] text-zinc-300 dark:text-zinc-600">
                    ({linkedEmails.filter((e) => e.email_type === "correspondence").length})
                  </span>
                </div>
                {linkedEmails
                  .filter((e) => e.email_type === "correspondence")
                  .map((email) => (
                    <EmailRow key={email.id} email={email} onUnlink={handleUnlink} />
                  ))}
              </div>
            )}

            {/* Campaigns section */}
            {linkedEmails.filter((e) => e.email_type === "campaign").length > 0 && (
              <div>
                <div className="sticky top-0 z-10 flex items-center gap-1.5 px-4 py-1.5 bg-zinc-50/90 dark:bg-zinc-900/90 backdrop-blur-sm border-b border-zinc-100 dark:border-zinc-800/50">
                  ...
                  <span className="text-[10px] text-zinc-300 dark:text-zinc-600">
                    ({linkedEmails.filter((e) => e.email_type === "campaign").length})
                  </span>
                </div>
                {linkedEmails
                  .filter((e) => e.email_type === "campaign")
                  .map((email) => (
                    <EmailRow key={email.id} email={email} onUnlink={handleUnlink} />
                  ))}
              </div>
