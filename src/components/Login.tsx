...
// Login page with GitHub OAuth + Microsoft 365 OAuth
...
export function Login() {
  const { signInWithGitHub, signInWithMicrosoft, isLoading, error, clearError } = useAuth();
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl shadow-zinc-200/50 dark:shadow-none border border-zinc-200/50 dark:border-zinc-800 p-6">
            ...
            <div className="space-y-3">
              {/* Microsoft 365 — primary option */}
              <button
                onClick={() => signInWithMicrosoft()}
                disabled={isLoading}
                className="w-full flex items-center justify-center gap-3 bg-[#0078d4] hover:bg-[#106ebe] text-white font-medium py-3 px-4 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md"
              >
                {isLoading ? (
                ...
                <span>{isLoading ? "Opening browser..." : "Continue with Microsoft 365"}</span>
              </button>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 text-sm">
                ...
                <div>
                  <p className="font-medium text-zinc-700 dark:text-zinc-200">Team workspace access</p>
                  <p className="text-zinc-500 dark:text-zinc-400 text-xs mt-0.5">Sign in with your organization account</p>
                </div>
              </div>
              <div className="flex items-start gap-3 text-sm">
                ...
                <div>
                  <p className="font-medium text-zinc-700 dark:text-zinc-200">Identity linked</p>
                  <p className="text-zinc-500 dark:text-zinc-400 text-xs mt-0.5">GitHub and Microsoft accounts are automatically matched</p>
                </div>
              </div>
            </div>
          </div>
