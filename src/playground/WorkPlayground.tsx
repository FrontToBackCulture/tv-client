...
// Work module playground — Dashboard, Board, Tracker views
...
import {
  ...
  useStatuses, useTasks, useTask, useUsers,
} from "../hooks/useWork";
...
import {
  ...
  ProjectHealthColors, PriorityLabels,
...
// Types
// ============================
type WorkView = "inbox" | "dashboard" | "board" | "tracker";
...
// ============================
function useInitiativeProjects() {
  return useQuery({
...
function TrackerView({
  ...
  // Task counts for at-risk projects
  const projectTaskCounts = useMemo(() => {
    const map = new Map<string, { total: number; completed: number }>();
  ...
  // Stalled tasks (in progress, not updated in 7+ days)
...
// Task Quick Detail
// ============================
function TaskQuickDetail({ taskId, onClose }: { taskId: string; onClose: () => void }) {
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border-zinc-100 dark:border-zinc-800/50">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-xs text-zinc-400 tabular-nums flex-shrink-0">{getTaskIdentifier(task)}</span>
