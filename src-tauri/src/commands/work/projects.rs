...
pub async fn work_create_project(data: CreateProject) -> CmdResult<Project> {
    ...
    if data.project_type.as_deref() == Some("deal") {
        if let Some(ref cid) = data.company_id {
            let company: Option<crate::commands::crm::types::Company> = client
            ...
            if let Some(c) = company {
                if c.stage.as_deref() == Some("prospect") {
                    ...
                    let _: crate::commands::crm::types::Company = client
                    ...
                    let _: crate::commands::crm::types::Activity = client.insert("crm_activities", &activity).await?;
                }
            }
        }
    }
                ...
                if old_stage != new_stage {
                    ...
                    if let Some(ref cid) = current.company_id {
                        ...
                        if new_stage == "won" {
                            ...
                            let _: crate::commands::crm::types::Activity = client.insert("crm_activities", &company_activity).await?;
                        }
                    }
                }
            }
