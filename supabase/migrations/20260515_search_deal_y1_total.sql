-- Update search_all() so the deal subtitle prefers the derived
-- deal_year_1_total over the legacy deal_value column. Also surface
-- the MRR component when present so subscription deals are recognisable.
...
as $$
  select * from (
    ...
    select
      ...
      concat_ws(' · ',
        nullif(c.industry, ''),
        initcap(replace(coalesce(c.stage, ''), '_', ' '))
      ) as subtitle,
      ts_rank(
        to_tsvector('simple', coalesce(c.name,'') || ' ' || coalesce(c.industry,'') || ' ' || coalesce(c.notes,'')),
        plainto_tsquery('simple', query)
      ) as rank
    from crm_companies c
    where to_tsvector('simple', coalesce(c.name,'') || ' ' || coalesce(c.industry,'') || ' ' || coalesce(c.notes,''))
      @@ plainto_tsquery('simple', query)

    union all

    -- Contacts: company · role
    select
      'contact'::text,
      ct.id,
      ct.name,
    ...
    -- Deals: stage · y1_value (or legacy deal_value) · mrr · solution
    select
      'deal'::text,
      p.id,
      p.name,
      concat_ws(' · ',
        ...
        case when coalesce(p.deal_year_1_total, p.deal_value) is not null
          then coalesce(p.deal_currency, 'S$') || to_char(coalesce(p.deal_year_1_total, p.deal_value), 'FM999,999')
        end,
        case when p.deal_mrr is not null
          then coalesce(p.deal_currency, 'S$') || to_char(p.deal_mrr, 'FM999,999') || '/mo'
        end,
